...
export const handler = createNetlifyHandler({
  ...
  try {
    ...
    switch (action) {
      case 'generate': {
        ...
        if (stream) {
          const streamResponse = await databricksService.streamText({
            prompt,
          ...
          return new NextResponse(streamResponse.stream, {
            headers: {
              ...
              'X-Provider': 'databricks'
            },
          })
        } else {
          const result = await databricksService.generateText({
            prompt,
            options: { maxTokens, temperature }
          })
      ...
      case 'chat': {
        ...
        const result = await databricksService.getChatCompletion({
          messages,
          options: { maxTokens, temperature }
        })
    ...
    return jsonResponse(
      { error: error.message || 'Failed to process Databricks AI request' },
      { status: 500 }
    )
  }
}
  ...
  try {
    ...
    const databricksService = new DatabricksService()
    const isValid = databricksService.validateConfig()
    ...
    console.error('Databricks health check error:', error)
    return jsonResponse(
      { 
        provider: 'databricks',
        status: 'error',
