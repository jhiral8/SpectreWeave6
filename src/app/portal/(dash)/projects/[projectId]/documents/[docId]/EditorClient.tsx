...
function EditorClient({ projectId, docId }: { projectId: string; docId: string }) {
      ...
      try {
        ...
        if (incomingContent) {
          ...
          if (parsed && (parsed.manuscript || parsed.framework)) {
            if (parsed.manuscript) { try { ed.commands.setContent(parsed.manuscript) } catch { ed.commands.setContent(parsed.manuscript) } }
          } else if (looksLikeDoc) {
            try { ed.commands.setContent(parsed) } catch { ed.commands.setContent(parsed) }
          }
        }
      } catch (e: any) {
  ...
  React.useEffect(() => {
    ...
    const scheduleSave = (json: any) => {
      ...
      saveTimer.current = setTimeout(async () => {
        try {
          const w: any = window
          const manuscript = json
          const payload = {
            content: JSON.stringify({ mode: 'dual', manuscript }),
            manuscript_content: manuscript ?? null,
