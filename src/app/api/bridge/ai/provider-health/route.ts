...
export async function GET(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${BACKEND_ORIGIN}/api/ai/provider-health`, { headers: authHeader ? { Authorization: authHeader } : {} })
    const data = await res.json().catch(() => ({}))
    return NextResponse.json(data, { status: res.status })
  } catch (e: any) {
    return NextResponse.json({ success: false, error: e?.message || 'Failed to fetch provider health' }, { status: 500 })
  }
}
