...
export const POST = createProtectedRoute(async (_ctx, req: NextRequest, _perm, { params }: { params: { id: string } }) => {
  ...
  try {
    ...
    const res = await fetch(`${BACKEND_ORIGIN}/api/chapters/generations/${id}/resume`, {
      method: 'POST',
    ...
    return NextResponse.json({ success: false, error: e?.message || 'Failed to resume' }, { status: 500 })
  }
}, { requireAuth: true })
