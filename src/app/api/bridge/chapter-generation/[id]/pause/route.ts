...
export const POST = createProtectedRoute(async (_ctx, req: NextRequest, _perm, { params }: { params: { id: string } }) => {
  ...
  try {
    ...
    const res = await fetch(`${BACKEND_ORIGIN}/api/chapters/generations/${id}/pause`, {
      method: 'POST',
    ...
    return NextResponse.json({ success: false, error: e?.message || 'Failed to pause' }, { status: 500 })
  }
}, { requireAuth: true })
