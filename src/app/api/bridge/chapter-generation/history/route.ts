...
export const GET = createProtectedRoute(async (_ctx, req: NextRequest) => {
  try {
    ...
    const res = await fetch(`${BACKEND_ORIGIN}/api/chapters/generations${qs}`, {
      headers: authHeader ? { Authorization: authHeader } : {},
    })
    const data = await res.json().catch(() => ({}))
    return NextResponse.json(data, { status: res.status })
  } catch (e: any) {
    return NextResponse.json({ success: false, error: e?.message || 'Failed to fetch history' }, { status: 500 })
  }
}, { requireAuth: true })
