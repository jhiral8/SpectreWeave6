...
export async function GET(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${BACKEND_ORIGIN}/api/neo4j/constraints`, {
    ...
    return NextResponse.json({ success: false, error: e?.message || 'Failed to fetch constraints' }, { status: 500 })
  }
}
